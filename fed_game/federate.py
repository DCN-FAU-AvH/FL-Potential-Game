...
def FL_train(cfg, server, clients, dataset):
    ...
    for t in range(1, cfg.T + 1):  # Iterate over FL training rounds.
        log.info(f"\nRound: {t}")
        ...
        if t % cfg.save_freq == 0 or t in [1, cfg.T]:
            loss = evaluate(cfg, server.model, dataset, res_dict, log, t)
...
def cal_loss(cfg, model, dataset, bs=500):
    ...
    with torch.no_grad():
        ...
        for i in range(cfg.m):
            ...
            loss_i /= batch_idx + 1
            loss += cfg.rho[i] * loss_i  # total weighted loss
    return loss.item()
